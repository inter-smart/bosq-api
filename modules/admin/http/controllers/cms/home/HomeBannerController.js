...
class HomeBannerController {
  ...
  static async store(req, res) {
    ...
    try {
      ...
      const data = await sequelize.transaction(async (transaction) => {
        return await DataModel.create(req.body, { transaction });
      });

      await invalidateCache(cacheKey);

      return sendSuccessResponse(res, data, "Data created successfully", 201);
    } catch (error) {
  ...
  static async update(req, res) {
    ...
    try {
      ...
      return sendSuccessResponse(res, data, "Data updated successfully");
    } catch (error) {
      console.error("Data update error:", error);
      return sendErrorResponse(res, error);
    }
